...
export default function EmployeePortal() {
  ...
  const handleUnlock = (e: React.FormEvent) => {
    e.preventDefault();
    if (pinCode === '2026') { setIsUnlocked(true); } else { setError(true); setPinCode(''); }
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0f0f0f] pt-20">
        <div className="bg-[#1a1a1a] p-8 rounded-xl shadow-md max-w-sm w-full text-center border border-white/10">
          ...
          <form onSubmit={handleUnlock}>
            <input type="password" value={pinCode} onChange={(e) => setPinCode(e.target.value)} className="w-full text-center text-2xl p-3 bg-[#0a0a0a] border border-white/20 text-white rounded-lg mb-4" placeholder="••••" />
            <button type="submit" className="w-full bg-[#00A0A0] text-white py-3 rounded-lg hover:bg-[#007a52] transition-colors">Access Portal</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#0f0f0f] pt-28 pb-12">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="relative mb-8">
          ...
          <input type="text" placeholder="Search documents..." className="w-full pl-12 pr-4 py-4 bg-[#1a1a1a] border border-white/10 text-white rounded-xl" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <div className="grid gap-4">
          {loading ? <div className="text-white">Loading...</div> : filteredDocs.map(doc => {
            const Icon = getIconForCategory(doc.category);
            return (
              <a key={doc.slug} href={doc.documentFile} target="_blank" rel="noopener noreferrer" className="p-6 border border-white/5 rounded-xl hover:bg-white/5 bg-[#1a1a1a] flex items-center justify-between group">
